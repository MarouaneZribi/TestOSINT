...
original_author="MarouaneZribi"
original_name="testosint"
original_urlname="TestOSINT"
original_description="Awesome testosint created by MarouaneZribi" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/testosint "${name}"
...
then
    ...
    echo "then run 'testosint' or 'python -m testosint'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'testosint' or 'python -m testosint' or 'poetry run testosint'"
fi
