...
- 🎯 Entry points to execute your program using `python -m <testosint>` or `$ testosint` with basic CLI argument parsing.
...
---
# testosint Flask Application

Awesome testosint created by MarouaneZribi
...
```bash
git clone https://github.com/MarouaneZribi/TestOSINT testosint
cd testosint
make install
```
...
```bash
testosint create-db   # run once
testosint populate-db  # run once (optional)
testosint add-user -u admin -p 1234  # ads a user
testosint run
```
